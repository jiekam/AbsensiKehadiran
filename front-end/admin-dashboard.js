const API_URL = 'https://wilful-berty-smkn1-97e664f8.koyeb.app';
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    const pendaftaranTab = document.getElementById('pendaftaranTab');
    if (pendaftaranTab) {
        pendaftaranTab.addEventListener('click', () => switchPage('pendaftaran'));
    }

    // Refresh RFID Button
    const refreshRfidBtn = document.getElementById('refreshRfidBtn');
    if (refreshRfidBtn) {
        refreshRfidBtn.addEventListener('click', () => {
            refreshRfidBtn.classList.add('refreshing');
            loadLatestRfid();
            setTimeout(() => {
                refreshRfidBtn.classList.remove('refreshing');
            }, 1000);
        });
    }
    ...
    if (pendaftaranForm) {
        pendaftaranForm.addEventListener('submit', handlePendaftaranSubmit);
    }
    ...
    if (historySearchInput) {
        historySearchInput.addEventListener('input', (e) => {
            filterHistoryData(e.target.value);
        });
    }
    ...
    if (siswaSearchInput) {
        siswaSearchInput.addEventListener('input', (e) => {
            filterSiswaData(e.target.value);
        });
    }

    const statistikSearchInput = document.getElementById('statistikSearchInput');
    if (statistikSearchInput) {
        statistikSearchInput.addEventListener('input', (e) => {
            filterStatistikData(e.target.value);
        });
    }
});
...
async function loadHistoryData() {
    ...
    try {
        ...
        filteredHistoryData = [...historyData]; // Initialize filtered data
...
function renderHistoryTable() {
    ...
    const dataToRender = filteredHistoryData.length > 0 ? filteredHistoryData : historyData;
        ...
        tableBody.innerHTML = `
            <tr>
                <td colspan="9" style="text-align: center; padding: 40px; color: var(--text-light);">
                    ${hasSearch ? 'Tidak ada data yang sesuai dengan pencarian' : 'Tidak ada data history untuk tanggal yang dipilih'}
                </td>
            </tr>
        `;
        return;
...
function switchPage(page) {
    ...
    const pendaftaranPage = document.getElementById('pendaftaranPage');
    ...
    if (pendaftaranTab) pendaftaranTab.classList.remove('active');
    ...
    if (pendaftaranPage) pendaftaranPage.classList.remove('active');
    ...
    if (page === 'dashboard') {
        ...
        if (typeof loadStatistikSiswa === 'function') {
            loadStatistikSiswa();
        }
    } else if (page === 'pendaftaran') {
        if (pendaftaranTab) pendaftaranTab.classList.add('active');
...
function filterStatistikData(searchTerm) {
    if (!searchTerm || searchTerm.trim() === '') {
        filteredStatistikSiswaData = [];
        // Clear filter - show all data by re-rendering
        if (statistikSiswaData.length > 0) {
            renderStudentCards(statistikSiswaData);
        }
    } else {
        const term = searchTerm.toLowerCase().trim();
        filteredStatistikSiswaData = statistikSiswaData.filter(siswa => {
            const nama = (siswa.nama || '').toLowerCase();
            return nama.includes(term);
        });
        renderStudentCards(filteredStatistikSiswaData);
    }
}
...
// Statistik Siswa Management
let statistikSiswaData = [];
let filteredStatistikSiswaData = [];
...
// Filter Siswa Data
function filterSiswaData(searchTerm) {
    if (!searchTerm || searchTerm.trim() === '') {
...
function renderSiswaTable() {
    ...
    const dataToRender = filteredSiswaData.length > 0 ? filteredSiswaData : siswaData;
        ...
        tableBody.innerHTML = `
            <tr>
                <td colspan="6" style="text-align: center; padding: 40px; color: var(--text-light);">
                    ${hasSearch ? 'Tidak ada data yang sesuai dengan pencarian' : 'Tidak ada data siswa'}
                </td>
            </tr>
        `;
        return;
    ...
    dataToRender.forEach((siswa, index) => {
        // Find original index in siswaData for proper data mapping
        const originalIndex = siswaData.findIndex(s => s.id === siswa.id);
        const displayIndex = originalIndex !== -1 ? originalIndex : index;
        const row = document.createElement('tr');
        row.innerHTML = `
            <td>
                <input type="checkbox" class="row-checkbox" data-index="${displayIndex}">
            </td>
            <td>
                <div class="id-cell">
            ...
            <td>
                <input type="text" class="editable-input" data-field="nama" data-index="${displayIndex}" value="${siswa.nama || ''}" readonly title="Double-click untuk edit">
            </td>
            <td>
                <input type="number" class="editable-input" data-field="nis" data-index="${displayIndex}" value="${siswa.nis || ''}" readonly title="Double-click untuk edit">
            </td>
            <td>
                <input type="text" class="editable-input" data-field="rfid" data-index="${displayIndex}" value="${siswa.rfid || ''}" readonly title="Double-click untuk edit">
            </td>
            <td>
                <input type="text" class="editable-input" data-field="role" data-index="${displayIndex}" value="${siswa.role || ''}" readonly title="Double-click untuk edit">
            </td>
            <td>
                <input type="text" class="editable-input" data-field="password" data-index="${displayIndex}" value="${siswa.password || ''}" readonly title="Double-click untuk edit">
            </td>
        `;
        tableBody.appendChild(row);
    });
...
async function loadStatistikSiswa() {
    ...
    showLoading('Memuat data statistik siswa...');
    ...
    try {
        ...
        statistikSiswaData = siswaList;
        filteredStatistikSiswaData = [];
        
        // Reset search input
        const statistikSearchInput = document.getElementById('statistikSearchInput');
        if (statistikSearchInput) {
            statistikSearchInput.value = '';
        }
...
function renderStudentCards(siswaList) {
    ...
    if (siswaList.length === 0) {
        const searchInput = document.getElementById('statistikSearchInput');
        const hasSearch = searchInput && searchInput.value.trim() !== '';
        cardsGrid.innerHTML = `
